...
**Dry yeast rehydration:** `YeastRehydrate` (no input volume, water + yeast → `WORT` liquor) → normal `Combine` with main `WORT` (`pitchCombine` optional; both streams are `WORT`) → `PRIMARY`. Do not require `pitchCombine` for rehydration; use it only when pitching fermented starter **beer** into main wort. Existing recipes may still use `Stand` for the same behaviour.
