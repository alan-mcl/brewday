...
public class SwingDuplicateRecipeDialog extends JDialog
{
	private Recipe output;
	private final JTextField recipeName;
	private final JLabel warningLabel;
	private final JButton okButton;
	private final Recipe current;

	public SwingDuplicateRecipeDialog(JFrame parent, Recipe current)
	{
		super(parent, getUiString("recipe.duplicate"), true);
		...
		ActionHotkeySupport.bind(getRootPane(), javax.swing.KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0), "dupRecipe.cancel", new AbstractAction()
		{
			@Override
		...
		ActionHotkeySupport.bind(getRootPane(), ActionHotkeySupport.ctrlOrCmd(KeyEvent.VK_ENTER), "dupRecipe.ok", new AbstractAction()
		{
			@Override
			public void actionPerformed(java.awt.event.ActionEvent e)
			{
	...
	private void updateOkState()
	{
		...
		boolean exists = !empty && Database.getInstance().getRecipes().containsKey(newValue.trim());
		if (empty)
		{
	...
	private void onOk()
	{
		...
		output.setName(recipeName.getText().trim());
		dispose();
	}
