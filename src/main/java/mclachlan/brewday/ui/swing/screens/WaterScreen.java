...
public class WaterScreen extends JPanel implements SwingScreen
{
	...
	WaterScreen(JFrame parent, DirtyStateService dirtyState, DialogPort dialogPort, DbPort dbPort, RenameHook renameHook)
	{
		...
		table.getColumnModel().getColumn(0).setCellRenderer(new IngredientNameTableCellRenderer(
			modelRow -> SwingIcons.tableNavIcon(ScreenKey.WATER),
	...
	private void wireHotkeys()
	{
		...
		ActionHotkeySupport.bind(this, ActionHotkeySupport.ctrlOrCmd(KeyEvent.VK_D), "water.hotkey.duplicateCtrl", duplicateAction);
	...
	private void duplicateSelected()
	{
		...
		Water draft = new Water(current);
		draft.setName("");
		Water created = dialogPort.showEditWaterDialog(parent, draft, true);
		if (created == null)
		{
			return;
		}
